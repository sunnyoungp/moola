...
- **Optional Object Safety**: Always use null-checks or optional chaining (`?.`) when accessing properties of objects returned from `find`, `filter`, or helper functions (e.g., `homeRange`). Accessing a property of `null` or `undefined` triggers a `TypeError` that crashes the rendering cycle and leads to blank tabs.
...
- **Row Object Variable Scope (CRITICAL)**: Any variable referenced in `out.push({...})` inside `ftCalcRows` **MUST** be declared at the outer function scope (alongside `let requiredSavings`, `let noGoals`, etc.) — NOT inside a conditional branch (`if/else/else if`). A `const/let` inside a branch is block-scoped: when `out.push` executes for a row that took a different branch (rate-mode, history), the reference throws a `ReferenceError` that silently kills the entire projection loop. Symptom: **Life Plan tab renders completely blank with no visible error message.** Rule: always hoist the declaration above the if/else chain and initialize to a safe default (`[]`, `null`, `false`, `0`).
